...
class DS {
    ...
    static function connect($host,$user,$pass,$database) {
        ...
        DS::$connections[$database]["connection"] = new PDO("sqlsrv:Server={$host};Database={$database}", $user, $pass);
    ...
    static function query($query) {
        ...
        $connection = DS::get(DS::$default_database);
        ...
        while($pos = stripos($query, "?", $offset)) {
            ...
            // ?s = strings
            if($type=="s") {
                $queryArgs[] = mysqli_real_escape_string($connection, $arguments[$num]);
            }
        ...
        if($result) {
            ...
            die("Error in query: {$query}</br> - ".print_r($connection->errorInfo(),true));
        }
        return null;
    }
